...
module.exports = {
	...
	async execute(interaction) {
        let servObj = defaultQuipCooldowns.filter(data => data.server == interaction.guild.id);
        servObj.at(0).defaultQuipCooldown = interaction.options.getNumber('cooldown');

        file.set("defaultQuipCooldowns", defaultQuipCooldowns.map(obj => servObj.find(o => o.server === obj.server) || obj));
        file.save();
